...
// flow above). spa and blazor need a WebApi host (cqrs/es tiers only); wpf
// works on cqrs/es (HTTP, calling the WebApi) or dm (in-process, calling the
// CrudService directly - dm has no WebApi host); mvc is dm-only, in-process.
