...
SOURCE = ROOT / "aspgen-wpfui-ddd-developer-guide.md"
HTML = ROOT / "aspgen-wpfui-ddd-developer-guide.html"
PDF = ROOT / "aspgen-wpfui-ddd-developer-guide.pdf"
TITLE = "aspgen WPF-UI Fullstack DDD Developer Guide"
SUBTITLE = "A practical, step-by-step guide to building and extending a fullstack (WebApi + WPF-UI desktop) Clean Architecture/DDD application with aspgen."
...
def highlight(code):
    ...
    text = re.sub(r"\b(aspgen|new|add|class|public|private|sealed|record|return|async|await|var|if|true|false|void|using)\b", r'<span class="kw">\1</span>', text)
    return text
...
</style></head><body><div class="page"><p class="mast">DEVELOPER GUIDE</p><h1>{TITLE}</h1><p>{SUBTITLE}</p><nav class="toc"><h2>Contents</h2><ul>{toc_html}</ul></nav><main>{''.join(content)}</main><footer>Architecture Theta &bull; Version 1.0 &bull; Prepared 04 August 2026</footer></div></body></html>'''
...
def pdf_output():
    ...
    story = [Paragraph(TITLE, styles["h1"]), Paragraph(SUBTITLE, styles["body"]), Spacer(1, 10)]
    first_h1 = True
    for block in blocks():
        kind = block[0]
        if kind == "h1":
            ...
            for item in block[1]:
                story.append(Paragraph("&bull; " + inline(item), styles["body"]))
        elif kind == "numbers":
    ...
    def footer(canvas, doc):
        ...
        canvas.drawRightString(7.5 * inch, .5 * inch, f"Architecture Theta  \u2022  Page {doc.page}")
        canvas.restoreState()

    SimpleDocTemplate(str(PDF), pagesize=LETTER, leftMargin=inch, rightMargin=inch, topMargin=.7 * inch, bottomMargin=.7 * inch, title=TITLE).build(story, onFirstPage=footer, onLaterPages=footer)
