...
SOURCE = ROOT / "aspgen-generation-decision-guide.md"
HTML = ROOT / "aspgen-generation-decision-guide.html"
PDF = ROOT / "aspgen-generation-decision-guide.pdf"
...
def highlight(code):
    ...
    text = re.sub(r"\b(aspgen|new|add|class|public|private|sealed|return|async|await|var|if|true|false|void|using)\b", r'<span class="kw">\1</span>', text)
    return text
...
def html_output():
    ...
    html = f'''<!doctype html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>aspgen Generation Decision Guide</title><style>
...
</style></head><body><div class="page"><p class="mast">DECISION GUIDE</p><h1>aspgen Generation Decision Guide</h1><p>Flags, generated trees, dependency direction, code shapes, and selection rules for Architecture Theta.</p><nav class="toc"><h2>Contents</h2><ul>{toc_html}</ul></nav><main>{''.join(content)}</main><footer>Architecture Theta • Version 1.0 • Prepared 03 August 2026</footer></div></body></html>'''
...
def pdf_output():
    ...
    story = [Paragraph("aspgen Generation Decision Guide", styles["h1"]), Paragraph("Flags, generated trees, dependency direction, code shapes, and selection rules for Architecture Theta.", styles["body"]), Spacer(1, 10)]
    first_h1 = True
    for block in blocks():
        kind = block[0]
        if kind == "h1":
    ...
    SimpleDocTemplate(str(PDF), pagesize=LETTER, leftMargin=inch, rightMargin=inch, topMargin=.7 * inch, bottomMargin=.7 * inch, title="aspgen Generation Decision Guide").build(story, onFirstPage=footer, onLaterPages=footer)
